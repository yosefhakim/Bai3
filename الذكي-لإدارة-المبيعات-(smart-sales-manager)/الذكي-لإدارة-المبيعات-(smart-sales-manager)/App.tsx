...
import { LayoutDashboard, Package, ShoppingCart, Users, LineChart, Menu, X, Receipt, Sparkles, Wifi, WifiOff } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'chat': return <SmartChat products={products} customers={customers} onUpdate={loadData} />;
      ...
      <div className={`${sidebarOpen ? 'w-72' : 'w-0 overflow-hidden'} transition-all duration-300 bg-slate-900 h-full fixed md:relative z-40 border-l border-slate-800 flex flex-col`}>
        <div className="p-8 flex-1 flex flex-col">
          ...
          <nav className="space-y-2 flex-1">
            ...
            <SidebarItem active={view === 'chat'} onClick={() => setView('chat')} icon={<Sparkles />} label="المساعد الذكي" />
          </nav>
